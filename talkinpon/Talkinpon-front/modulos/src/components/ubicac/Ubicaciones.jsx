...
export default function Ubicaciones() {
  ...
  */
  ...
  // uso de sendUsuarioMensaje para mandar el mensaje del Usuario
  const handleSendMessage = async () => {
    if (!inputValue.trim()) return;

    const userMessage = { type: "user-ub", content: inputValue, timestamp: new Date() };
    ...
    try {
      const data = await sendUsuarioMensaje(inputValue, "ubicaciones"); // Llamada a Django
      if (data && data.reply) {
        const botMessage = { type: "bot", content: data.reply, timestamp: new Date() };
        setMessages((prev) => [...prev, botMessage]);
      }
    } catch (err) {
      console.error(err);
      const errorMessage = { type: "bot", content: "No se pudo conectar con el servidor.", timestamp: new Date() };
