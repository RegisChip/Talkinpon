# Talkinpon\Talkinpon\Ubicaciones\views.py
...
from Chat.ollama_servidor import respuesta as respuesta_ollama  # NUEVO: Importar IA
import heapq
import json
import math

# ============================================================================
# VISTAS PARA EL MAPA MODAL (AdminU React)
# ============================================================================

def mapa(request):
    """Vista para el mapa HTML original (mapa.html)"""
...
def datos_mapa(request):
    """Devuelve todas las ubicaciones y relaciones para el mapa modal"""
    try:
        ubicaciones = list(Ubicacion.objects.values(
            'id_ubicacion', 'pos_x', 'pos_y', 'tipo', 'nom_nodo'
        ))
        
        # Renombrar id_ubicacion a id para compatibilidad con React
        for u in ubicaciones:
            u['id'] = u.pop('id_ubicacion')
        
        relaciones = list(RelacionU.objects.values('id', 'origen_id', 'destino_id'))
...
def crear_nodo(request):
    """Crea un nodo intermedio en el mapa"""
    try:
...
def crear_relacion(request):
    ...
    try:
        ...
        if not origen_id or not destino_id:
            return JsonResponse({'error': 'IDs de nodos requeridos'}, status=400)
        
        # Obtener nodos
        origen = Ubicacion.objects.get(id_ubicacion=origen_id)
        destino = Ubicacion.objects.get(id_ubicacion=destino_id)
        
        # Validación: no permitir edificio → edificio
        if origen.tipo == 'edificio' and destino.tipo == 'edificio':
            return JsonResponse({
                'error': 'No se puede conectar dos edificios directamente. Usa nodos intermedios.'
            }, status=400)
        
        # Crear o obtener relación
        relacion, created = RelacionU.objects.get_or_create(
            origen=origen,
            destino=destino
        )
        ...
        return JsonResponse({
            ...
            'relacion': {
                'id': relacion.id,
                'origen_id': origen.id_ubicacion,
                'destino_id': destino.id_ubicacion
            }
        })
        
    except Ubicacion.DoesNotExist:
        return JsonResponse({'error': 'Uno o ambos nodos no existen'}, status=404)
    except Exception as e:
        return JsonResponse({'error': str(e)}, status=500)
...
def eliminar_nodo(request):
    ...
    try:
        ...
        if not id_nodo:
            return JsonResponse({'error': 'ID de nodo requerido'}, status=400)
        ...
        # Si es edificio, eliminar edificio asociado
...
def eliminar_relacion(request):
    ...
    try:
        ...
        if not id_relacion:
            return JsonResponse({'error': 'ID de relación requerido'}, status=400)
        
        # Obtener y eliminar relación
        relacion = RelacionU.objects.get(id=id_relacion)
        
        # Si es bidireccional, eliminar también la relación inversa
        if relacion.bidireccional:
            RelacionU.objects.filter(
                origen=relacion.destino,
                destino=relacion.origen,
                bidireccional=False
            ).delete()
...
# ============================================================================
# VISTAS ORIGINALES (Mapa HTML anterior)
# ============================================================================

def guardar_ubicacion(request):
    """Vista original para el mapa HTML"""
...
def obtener_edificios_json(request):
    """Devuelve todos los edificios en formato JSON para React"""
    print("\n" + "="*80)
...
# ============================================================================
# ALGORITMO DIJKSTRA Y RUTAS (CON INTEGRACIÓN DE IA - VERSIÓN FUSIONADA)
# ============================================================================
...
def dijkstra_ruta(request):
    ...
    VERSIÓN FUSIONADA: Incluye generación de descripción con IA
    """
    print("\n" + "="*80)
    ...
    json_ruta['instrucciones'] = instrucciones
    json_ruta['nodos_visitados'] = len(camino_coordenadas)
    json_ruta['camino_coordenadas'] = camino_coordenadas
    json_ruta['distancia_total'] = distancia_total
    
    # NUEVO: Obtener la imagen del edificio destino
    imagen_destino_url = None
    if hasattr(edificio_destino, 'edificio_imagen') and edificio_destino.edificio_imagen:
        imagen_destino_url = edificio_destino.edificio_imagen.url
        print(f"🖼️  IMAGEN DESTINO: {imagen_destino_url}")
    else:
        print(f"⚠️  El edificio {edificio_destino.nombre} no tiene imagen configurada")
    
    json_ruta['destino']['imagen_url'] = imagen_destino_url
    
    # Mostrar JSON para IA en consola
    json_para_consola = {
    ...
    # ====== NUEVO: BLOQUE DE INTEGRACIÓN CON OLLAMA ======
    descripcion_ia = "No se pudo generar una descripción detallada en este momento."
...
def construir_grafo():
    """Construye el grafo completo usando TODAS las relaciones"""
    grafo = {}
    relaciones = RelacionU.objects.select_related('origen', 'destino').all()
    ...
    for rel in relaciones:
        ...
        if tipo_origen == 'edificio' and tipo_destino == 'edificio':
            ...
            print(f"⚠️  Ignorando conexión directa: {rel.origen.nom_nodo} → {rel.destino.nom_nodo}")
            continue
    ...
    print(f"\n📊 RESUMEN del grafo:")
    print(f"   - Total nodos: {len(grafo)}")
    print(f"   - Total conexiones: {contador_relaciones}")
    print(f"   - Relaciones ignoradas: {relaciones_ignoradas}\n")
...
def generar_json_ruta(camino_coordenadas, ubicaciones_data, origen, destino):
    """
    Genera el JSON completo de la ruta con toda la información necesaria
    """
    puntos_asociados = []
    tipos = []
    direccion = []
    ...
    # Obtener TODOS los edificios con sus nombres reales
...
def generar_instrucciones(camino_coordenadas):
    ...
    for i in range(1, len(camino_coordenadas) - 1):
        ...
        if nodo['tipo'] == 'edificio':
            instrucciones.append(f"Dirígete {direccion} ({distancia:.0f}m) hacia {nodo['nom_nodo']}")
        else:
            instrucciones.append(f"Continúa {direccion} ({distancia:.0f}m) por {nodo['nom_nodo']}")
    ...
    instrucciones.append(f"Dirígete {direccion} hasta tu destino: {ultimo_nodo['nom_nodo']}")
