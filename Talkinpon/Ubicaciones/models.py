...
class Edificio(models.Model):
    ...
    nombre_especial = models.CharField(max_length=150, blank=True, null=True)  # NUEVO
    ...
    # ruta de la imagen
    edificio_imagen = models.ImageField(upload_to='images/', blank=True, null=True)
...
class TipoArea(models.Model):
    """Tipos de área (departamento, cubículo, laboratorio, sala especial...)"""
...
class AreaEdificio(models.Model):
    """Áreas, departamentos y espacios dentro de un edificio"""
    id_area = models.AutoField(primary_key=True)
    edificio = models.ForeignKey(Edificio, on_delete=models.CASCADE, related_name='areas')
    tipo = models.ForeignKey(TipoArea, on_delete=models.CASCADE, related_name='areas')
    ...
    def __str__(self):
        return f"{self.nombre} ({self.tipo.nombre}) - {self.edificio.nombre}"